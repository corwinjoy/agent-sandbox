...
has     "proxy runs with no capabilities"  "--cap-drop=ALL --security-opt=no-new-privileges -e PROXY_MODE=enforce -e PROXY_INSPECT=none" "$PROXYCMD"
has_not "proxy adds no capabilities"       "--cap-add" "$PROXYCMD"
has     "proxy has its CA volumes"         "-v agent-proxy-ca:/var/lib/agent-proxy -v agent-proxy-ca-pub:/ca-pub" "$PROXYCMD"
has_not "trusted agent gets no CA mount"   "/etc/agent-sandbox/ca" "$AGENT"
has_not "trusted agent gets no CA env"     "SSL_CERT_FILE" "$AGENT"
...
has     "proxy inspects GitHub"            "-e PROXY_MODE=enforce -e PROXY_INSPECT=github" "$PROXYCMD"
has     "agent mounts the proxy CA read-only" "-v agent-proxy-untrusted-ca-pub:/etc/agent-sandbox/ca:ro" "$AGENT"
has     "git trusts the proxy CA"          "GIT_SSL_CAINFO=/etc/agent-sandbox/ca/ca-bundle.pem" "$AGENT"
has     "curl trusts the proxy CA"         "CURL_CA_BUNDLE=/etc/agent-sandbox/ca/ca-bundle.pem" "$AGENT"
has     "node trusts the proxy CA"         "NODE_EXTRA_CA_CERTS=/etc/agent-sandbox/ca/ca.pem" "$AGENT"
has     "python trusts the proxy CA"       "REQUESTS_CA_BUNDLE=/etc/agent-sandbox/ca/ca-bundle.pem" "$AGENT"
...
launcher "$T/proj" --untrusted --audit-egress; eq "--untrusted --audit-egress is refused" 2 "$RC"

section "agent-run.sh: audit mode"
launcher "$T/proj" --audit-egress
eq      "exits 0" 0 "$RC"
has     "warns loudly"                     "AUDIT MODE" "$OUT"
has     "uses its own proxy container"     "--name agent-proxy-audit" "$PROXYCMD"
has     "proxy allows and logs everything" "-e PROXY_MODE=audit -e PROXY_INSPECT=none" "$PROXYCMD"
has     "agent uses the audit proxy's CA volume name" "agent-proxy-audit-ca" "$PROXYCMD"
has     "agent still on the internal network" "--network agent-internal" "$AGENT"
has_not "audit does not inspect, so no CA in the agent" "SSL_CERT_FILE" "$AGENT"
