...
cd untrusted/project && agent-run.sh --untrusted           # no token, no GPU, model API plus read-only GitHub
```
...
scripts/
  ...
  agent-run.sh                Launcher: --gpu, --perf, --ask, --untrusted, --shell, --check-token, --audit-egress
  egress-report.sh            What the proxy allowed and refused, per domain
  ...
  container/
    ...
    Containerfile.proxy       Egress proxy image (Squid with TLS support, unprivileged)
    proxy-entrypoint.sh       Picks enforce/audit mode, creates the inspection CA, starts Squid
    squid.conf                Proxy rules: HTTPS tunnels to allowlisted domains; read-only GitHub in untrusted mode
    mode-enforce.conf, mode-audit.conf, inspect-github.txt
    allowed-domains.txt       Allowlist, trusted mode
    allowed-domains-untrusted.txt   Allowlist, untrusted mode: Anthropic endpoints (GitHub is inspected, reads only)
...
slides/
  agent-sandbox-talk.odp      A 16-slide talk (LibreOffice Impress) with speaker notes
...
- It is a shared-kernel sandbox. A Linux kernel or NVIDIA driver bug can still reach the host.
  ...
  Code sandbox, plain Docker, gVisor, Docker Sandboxes, NVIDIA OpenShell and VMs, and says when to pick which.
...
- `github.com` is on the trusted-mode allowlist, so data can be sent there. The single-repo token
  limits where. In untrusted mode the proxy inspects GitHub traffic and refuses every write.
- The GitHub token is an environment variable inside the container, readable by anything that
  runs there. The guide's [Appendix F](docs/setup-guide.md#appendix-f-nvidia-openshell-in-detail)
  compares this with NVIDIA OpenShell, which keeps credentials out of the sandbox.
